...
| やりたいこと | 伝え方 |
|---|---|
| いま直した画面の証跡がほしい | コマンドの後ろには何も書かない。何を触っていたかは分かっています |
| MR/PR の証跡がほしい | リンクを貼る。説明と diff を読みます |
| 説明したとおりにページを録ってほしい | 上の例のように `Page: <url>` と手順を書く |
| 同じ録画をもう一度、またはもっとゆっくり | 頼むだけ。コマンドは runbook に残っていて、前の録画も上書きされません |
| 字幕を別の言語で | 言語を伝える。プロジェクトごとに覚えています |
| 一度だけ設定して、以降はログイン済みの状態から録りたい | `set up the evidence config for this project` |
