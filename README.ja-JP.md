...
- **カーソルが見えて、自然に動きます。** 曲線で動き、加速してから減速し、遠いクリックでは少し
  行き過ぎて戻ります。クリックごとに小さな波紋が出ます。
- **テンポは画面の内容に合わせます。** 画面遷移だけのクリックは速く進み、結果が出たところでは
  読める長さだけ止まります。
- **撮れないものは字幕で補います。** `<select>` やファイル選択ダイアログは OS が描画するため、
  ページの録画には入りません。画面下部の字幕が何を選んだかを示します。キーボードショートカット
  は、対象の要素の横にキーヒント（`⌘ + C`）が出ます。
- **ページ読み込みの待ち時間はカットされる**ので、作業が始まるところから動画が始まります。
...
注意: `install.sh` は常にデフォルトブランチから取得されるので、インストーラ自体の修正はそこに
マージされてから届きます。
...
**タスクやバグ修正の直後。** どの画面が変わったかはエージェントが把握しているので、後ろに何も
書く必要はありません:
...
**手元にあるのが MR/PR のリンクだけのとき。** 説明と diff を読んで、何を録るべきかを判断します:
...
準備もいりません。手順は自分が使っている言語で書いてかまいません。エージェントは同じ言語で
答えます。
...
| やりたいこと | コマンド |
|---|---|
| いま直した画面の証跡がほしい | `/webapp-evidence:recording` |
| MR/PR の証跡を、差分から判断して録ってほしい | `/webapp-evidence:recording <MR/PR の URL>` |
| リポジトリなしで、説明したとおりにページを録ってほしい | `/webapp-evidence:recording Page: <url>` と手順 |
| README 用に gif、自前のページ用に webm がほしい | `/webapp-evidence:recording -f gif` · `-f webm` |
| 動画を見ずに、何が映っているか知りたい | `/webapp-evidence:vision <mp4 ファイル>` |
| 最初に一度だけ設定して、以降はログイン済みで録りたい | `/webapp-evidence:recording set up the evidence config for this project` |
| 同じ内容をもう一度 — データが変わった、動画が壊れた | 頼むだけ。コマンドはランブックにあり、前の録画も残ります |
| もっとゆっくり、字幕を別の言語で | そう言えば `evidence.config.js` を書き換えます |
...
なります——確認でき、ランブックとも突き合わせられる時刻です。遷移の途中で崩れるレイアウトなど、
誰もスクリーンショットを撮ろうと思わなかったものを見つけられます。
