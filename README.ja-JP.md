...
**別の形式にする。** 録画は mp4 です。マージリクエストでも issue でもチャットでもそのまま再生
でき、3 つの形式のなかで一番軽いものです。別の形式が要るときは頼んでください:

```
/webapp-evidence:recording -f gif
```

README に貼るなら gif、自分で管理しているページなら webm が向きます。どちらもサイズは高くつき
——同じ録画の gif は数倍になります——ので、勝手に変換せず提案します。

**動画の中身を読む。** エージェントは動画を再生できないので、タイムスタンプ付きのシートに並べて
画像として読みます:

```
/webapp-evidence:vision <mp4 ファイル>
```

数秒に 1 フレーム、それぞれに `mm:ss` が入るので、指摘は「00:14 でヘッダーが表とかぶっている」
という形で返ります。動画で確認でき、ランブックの行とも突き合わせられる時刻です。誰もスクリーン
ショットを撮ろうと思わなかったもの——遷移の途中で崩れるレイアウト、出てすぐ消えるバナー——を
見つけるのに向いています。
